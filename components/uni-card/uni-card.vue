...
<script>
	export default {
		...
		props: {
			...
			extra: {type: String,default: ''}, // 扩展信息
			note: {type: String,default: ''}, // Tips
			thumbnail: {type: String,default: ''}, // 缩略图
			// 卡片模式 ， 可选值 basic：基础卡片 ；style ：图文卡片 ； title ：标题卡片
			mode: {type: String,default: 'basic'},
			isFull: {type: Boolean,default: false},// 内容区域是否通栏
			isShadow: {type: Boolean,default: false}// 是否开启阴影
		},
		methods: {
			onClick() {
				this.$emit('click')
